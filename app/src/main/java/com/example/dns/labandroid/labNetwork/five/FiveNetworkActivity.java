...
public class FiveNetworkActivity extends AppCompatActivity implements Response.Listener<String>,
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        String url = "https://my-json-server.typicode.com/dserbin96/json/users";
    ...
    public void onResponse(String response) {
        try {
            Collection<FiveViewModel> listJson = new ArrayList<>();
            JSONArray jsonArray = new JSONArray(response);
            for (int i = 0; i < jsonArray.length(); i++) {
                int id = jsonArray.getJSONObject(i).getInt("id");
                String name = jsonArray.getJSONObject(i).getString("name");
                String url = jsonArray.getJSONObject(i).getString("url");
                listJson.add(new FiveViewModel(id, name, url));
